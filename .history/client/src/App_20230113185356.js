	...
	return (
		<div className="App absolute top-0 right-0 bottom-0 left-0">
			<aside className=" bg-[var(--text-primary)] w-[260px] p-[10px]">
				<div
					className="p-[12px] border  rounded-[5px] text-left hover:bg-[rgba(255,255,255,0.1)] transition-all"
				...
				<div className="Models flex items-center mt-[24px] w-full">
					...
					>
						{models.map((model) => (
							<option className="bg-[var(--text-primary)]" id={model.id}>
								{model.id}
							</option>
						))}
					</select>
			...
			<main className="chatbox   relative h-full w-full transition-width flex flex-col overflow-hidden items-stretch flex-1">
				ScrollToBottom
				<ScrollToBottom className="chat-log text-left flex-1 overflow-hidden">
				<div className=" chat-log text-left flex-1 overflow-hidden">
...
const ChatMessage = ({ message}) => {
	return (
		<div
			className={` chat-message overscroll-auto ${
				message.user === "gpt" && "chatgpt	bg-[var(--text-secondary)]"
			}`}
		>
