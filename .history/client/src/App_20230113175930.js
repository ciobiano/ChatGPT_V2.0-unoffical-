	...
	return (
		<div className="App absolute top-0 right-0 bottom-0 left-0">
			<aside className=" bg-[var(--text-primary)] w-[260px] p-[10px]">
				<div className="p-[12px] border  rounded-[5px] text-left hover:bg-[rgba(255,255,255,0.1)] transition-all" onClick={clearChatLog}>
				...
				<div className="Models flex items-center mt-[24px] w-full">
					...
					>
						{models.map((model) => (
							<option  className="bg-[var(--text-primary)]"id={model.id}>{model.id}</option>
						))}
					</select>
				</div>
			</aside>
			<section className="chatbox overflow-contain flex-[1] relative ">
				<div className=" chat-log overflow-contain text-left">
